...
def put_rezultat():
    ...
    cur.execute("""
        UPDATE Rezultat SET swim = %s, division = %s, run = %s, name = %s, profession = %s, country = %s,
            age = %s, run_distance = %s, bib = %s, state = %s, bike = %s, gender_rank = %s, overall = %s,
            swim_distance = %s, overall_rank = %s, points = %s, t2 = %s, bike_distance = %s, t1 = %s, div_rank = %s
        WHERE id = %s
    """, (swim, division, run, name, profession, country, age, run_distance, bib, state, bike, gender_rank,
          overall, swim_distance, overall_rank, points, t2, bike_distance, t1, div_rank, id))
...
#desktop 2
@app.delete('/tekmovanjedelete/<int:id>')
def delete_tekmovanje(id):
...
@app.delete('/rezultatdelete/<int:id>')
...
@app.get("/rezultat_byname/<name>/<bib>")
def get_rezultat_byname(name, bib):
    query = "SELECT * FROM Rezultat WHERE name = %s AND bib = %s"
    cur.execute(query, (name, bib))
    row = cur.fetchone()
